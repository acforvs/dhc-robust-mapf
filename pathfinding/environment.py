...
class MovingAIBenchmarkingEnvironment(Environment):
    def __init__(
        self,
        num_agents: int = 5,
        map_filename: str = None,
        test_descriptions: list[movingai.TestDescription] = None,
        obs_radius: int = ENV_CONFIG["observation_radius"],
        reward_fn: dict = ENV_CONFIG["reward_fn"],
        should_init: bool = True,
    ):
        if test_descriptions is not None and len(test_descriptions) != num_agents:
            raise ValueError("Number of tests must be equal to num_agents")
        ...
        if should_init:
            self.map = movingai.read_map(map_filename)
            self.map_size = (len(self.map), len(self.map))
            self.agents_pos = np.array(
                [[descr.x0, descr.y0] for descr in test_descriptions]
            )
            self.goals_pos = np.array(
                [[descr.x1, descr.y1] for descr in test_descriptions]
            )
            self.get_heuri_map()
            self.last_actions = np.zeros(
                (self.num_agents, 5, 2 * obs_radius + 1, 2 * obs_radius + 1),
                dtype=np.bool,
            )
